...
/// <summary>
/// Shared fixture that extracts API from the CompiledMode/TypeScript fixture once.
...
/// These document the accuracy gap that a compiled extractor will close:
...
public class TypeScriptCompiledPrecisionTests : IClassFixture<TypeScriptCompiledFixture>
{
    ...
    ///
    /// The source extractor processes src/index.ts which re-exports from all
    /// sub-modules, making everything available in one unified surface.
    ///
    /// A compiled extractor processing dist/types/index.d.ts would see only
    ...
    public void SourceParser_CannotDistinguish_DefaultExports_FromPlatformSpecific()
    {
        ...
        {
            Assert.Fail("Compiled extractor should produce a 'default' condition module");
        }
    }
}
