...
/// <summary>
/// Tests that extractors/models handle malformed, empty, or truncated output gracefully.
...
public class MalformedOutputTests
{
    ...
    #region ExtractorResult Patterns

    [Fact]
    public void ExtractorResult_Failure_ContainsError()
    {
        var result = new ExtractorResult.Failure("Something went wrong");
        Assert.False(result.IsSuccess);
        Assert.Contains("Something went wrong", result.Error);
    }

    [Fact]
    public void ExtractorResult_Success_IsSuccess()
    {
        var index = new DotNet.ApiIndex { Package = "test", Namespaces = [] };
