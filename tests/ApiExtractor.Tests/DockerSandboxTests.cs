...
public class DockerSandboxTests : IDisposable
{
    ...
    {
        Environment.SetEnvironmentVariable($"{DockerSandbox.ImageEnvVarPrefix}PYTHON", "mcr.microsoft.com/python-extractor:v2");

        Assert.Equal("mcr.microsoft.com/python-extractor:v2", DockerSandbox.GetImageName("python"));
    }
    ...
    [InlineData("ghcr.io/microsoft/python-extractor:v1")]
    [InlineData("ghcr.io/azure/api-extractor:latest")]
    [InlineData("api-extractor-python:latest")]
    ...
    [InlineData("my-registry/python-extractor:v2")]
