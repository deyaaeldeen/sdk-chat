...
- Base path: `azure-sdk-tools/tools/sdk-chat/`
...
LOCATION:  tools/sdk-chat/Sdk.Tools.Chat/
BUILD:     dotnet build tools/sdk-chat/Sdk.Tools.Chat/Sdk.Tools.Chat.csproj
RUN:       dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- <command>

ACP SDK:   AgentClientProtocol.Sdk (standalone, extractable)
LOCATION:  tools/sdk-chat/AgentClientProtocol.Sdk/
...
│   └── ConnectionTests.cs
│
├── Sdk.Tools.Chat/                     # ← APPLICATION (uses ACP SDK)
│   ├── Sdk.Tools.Chat.csproj           # References AgentClientProtocol.Sdk
│   ├── Acp/
...
| **SDK Chat (Phase 1 - Core)** |
| 01 | `Sdk.Tools.Chat.csproj` | 1 | `dotnet restore` succeeds |
...
| 07 | `Models/SdkChatConfig.cs` | 1 | compiles |
...
| 43 | `Sdk.Tools.Chat.Tests/Sdk.Tools.Chat.Tests.csproj` | 4 | `dotnet build` succeeds |
| 44 | `Sdk.Tools.Chat.Tests/LanguageDetectorTests.cs` | 4 | tests pass |
...
dotnet build tools/sdk-chat/Sdk.Tools.Chat/Sdk.Tools.Chat.csproj
dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- package sample generate --help

# Phase 2: All modes work
dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- acp --help
dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- mcp --help

# Phase 3: All languages detected
# (manual verification with test repos)

# Phase 4: Tests pass
dotnet test tools/sdk-chat/Sdk.Tools.Chat.Tests/
```
...
All paths relative to: `tools/sdk-chat/`
...
### VALIDATE
```bash
dotnet restore tools/sdk-chat/AgentClientProtocol.Sdk/AgentClientProtocol.Sdk.csproj
```
...
### VALIDATE
```bash
dotnet build tools/sdk-chat/AgentClientProtocol.Sdk/AgentClientProtocol.Sdk.csproj
```
...
# PHASE 1 FILES (SDK Chat Core)

All Phase 1 files go in: `Sdk.Tools.Chat/`

---

## FILE 01: Sdk.Tools.Chat.csproj
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <PropertyGroup>
    ...
    <ImplicitUsings>enable</ImplicitUsings>
    <Nullable>enable</Nullable>
    <RootNamespace>Sdk.Tools.Chat</RootNamespace>
  </PropertyGroup>
...
### VALIDATE
```bash
dotnet restore tools/sdk-chat/Sdk.Tools.Chat/Sdk.Tools.Chat.csproj
```
...
### VALIDATE
```bash
dotnet build tools/sdk-chat/Sdk.Tools.Chat/Sdk.Tools.Chat.csproj --no-restore
```
...
> **Source:** From `azsdk-chat/Services/Languages/Samples/SampleConstants.cs`
...
## FILE 07: Models/SdkChatConfig.cs
...
> **Architecture Note:** Adapted from battle-tested `azsdk-chat/Helpers/FileHelper.cs`. 
...
public class Program
{
    ...
    {
        var rootCommand = new RootCommand("SDK Chat - Tools for SDK development");
        
        var services = ConfigureServices();
        
        // sdk-chat package sample generate
...
public class SampleGeneratorAgent : IAgent
{
    ...
    public AgentInfo Info => new("sdk-chat", "1.0.0");
...
public static class Program
{
    public static async Task<int> Main(string[] args)
    {
        ...
        // sdk-chat mcp (MCP server for VS Code, Claude Desktop)
        rootCommand.AddCommand(BuildMcpCommand());
        
        // sdk-chat acp (ACP agent for interactive generation)
...
### VALIDATE
```bash
dotnet restore tools/sdk-chat/AgentClientProtocol.Sdk.Tests/AgentClientProtocol.Sdk.Tests.csproj
```
...
### VALIDATE
```bash
dotnet test tools/sdk-chat/AgentClientProtocol.Sdk.Tests/ --filter "FullyQualifiedName~JsonRpcMessageTests"
```
...
### VALIDATE
```bash
dotnet test tools/sdk-chat/AgentClientProtocol.Sdk.Tests/ --filter "FullyQualifiedName~NdJsonStreamTests"
```
...
### VALIDATE
```bash
dotnet test tools/sdk-chat/AgentClientProtocol.Sdk.Tests/ --filter "FullyQualifiedName~ConnectionTests"
```

### DONE WHEN
All ACP SDK tests pass:
```bash
dotnet test tools/sdk-chat/AgentClientProtocol.Sdk.Tests/
```

---

## FILE 43: Sdk.Tools.Chat.Tests/Sdk.Tools.Chat.Tests.csproj
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <ProjectReference Include="..\Sdk.Tools.Chat\Sdk.Tools.Chat.csproj" />
  </ItemGroup>
...
## FILE 44: Sdk.Tools.Chat.Tests/LanguageDetectorTests.cs
...
sdk-chat package sample generate --package-path /path/to/openai-dotnet

# Generate samples with custom prompt
sdk-chat package sample generate --package-path /path/to/sdk --prompt "Generate streaming examples"

# Dry run (print to console)
sdk-chat package sample generate --package-path /path/to/sdk --dry-run
```
...
### CLI Mode (Direct)
```bash
sdk-chat package sample generate --package-path ./my-sdk
```
...
```json
{
  "mcpServers": {
    "sdk-chat": {
      "command": "dotnet",
      "args": ["run", "--project", "path/to/Sdk.Tools.Chat", "--", "mcp"]
    }
  }
}
```
...
Create `sdk-chat-config.json` in your SDK root:
...
# Verify CLI
dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- --help
...
dotnet run --project tools/sdk-chat/Sdk.Tools.Chat -- package sample generate --package-path /tmp/openai-dotnet --dry-run
```
