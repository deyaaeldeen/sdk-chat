...
public static class ScriptUsageAnalyzerHelper
{
    ...
    public sealed record ScriptInvocationConfig
    {
        ...
        /// <summary>The availability result for the extractor runtime.</summary>
        public required ExtractorAvailabilityResult Availability { get; init; }
        ...
        /// </summary>
        public required Func<ExtractorAvailabilityResult, string, ScriptArgs> BuildArgs { get; init; }
        ...
        CancellationToken ct = default)
    {
        ...
        var result = JsonSerializer.Deserialize(output, ExtractorJsonContext.Default.UsageResult);
        if (result is null)
            return new AnalysisResult { Index = new UsageIndex { FileCount = 0 }, Errors = errors };
        ...
        if (availability.Mode == ExtractorMode.NativeBinary)
        {
            ...
            if (!result.Success)
            {
                errors.Add(result.TimedOut
                    ? $"{prefix}: timed out after {ExtractorTimeout.Value.TotalSeconds}s"
        ...
        if (availability.Mode == ExtractorMode.Docker)
        {
            ...
            if (!dockerResult.Success)
            {
                errors.Add(dockerResult.TimedOut
                    ? $"{prefix}: Docker timed out after {ExtractorTimeout.Value.TotalSeconds}s"
